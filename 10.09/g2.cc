...
int D[sap::MAXN], E[sap::MAXN];
int I[sap::MAXN], J[sap::MAXN], T[sap::MAXN];
...
int main(void)
{
    ...
    while (l < r)
    {
        int mid = (l + r) >> 1;
        sap::init(tot);
        for (int i = 1; i <= R; i++) sap::add_edge(Src, Ru(i), E[i]);
        for (int i = 1; i <= P; i++) sap::add_edge(Pu(i), Sink, D[i]);
        for (int i = 0; i < C; i++)
            if (T[i] <= mid)
                sap::add_edge(Ru(J[i]), Pu(I[i]), sap::inf);
