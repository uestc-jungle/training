        ...
        for (int i=0;i<=t;i++) {
            ...
            for (int j=1;j<=n;j++) {
              ...
              //          cout << i << ' ' << q << ' ' << j << ' ' << k << ' ' << l << ' ' << dp[i][j][k][l][q] << endl;
                            if (j==n) {
                                if (l==1&&q==0) ans=max(ans,dp[i][j][k][l][q]);
                                continue;
                            }
                        //stay here
                            for (int u=1;u<=K;u++) {
                                if (u!=l) {
                                    if (j==1||j==n) continue;
                                    dp[1+i][j][k][u][0]=max(dp[1+i][j][k][u][0],dp[i][j][k][l][q]);
                                } else if (q==0) {
                                    if (j==1||j==n) continue;
                                    if (p[u][j]<=dp[i][j][k][l][q]&&k+1<=b) {
                                        dp[i][j][k+1][u][1]=max(dp[i][j][k+1][u][1],dp[i][j][k][l][q]-p[u][j]);
                                    }
                                    if (k>0) {
                                        dp[i][j][k-1][u][1]=max(dp[i][j][k-1][u][1],dp[i][j][k][l][q]+p[u][j]);
                                    }
